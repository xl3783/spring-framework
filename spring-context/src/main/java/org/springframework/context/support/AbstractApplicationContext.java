...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public AbstractApplicationContext() {
		this.resourcePatternResolver = getResourcePatternResolver(); // 获取解析器
	}
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			prepareRefresh(); // 初始化监听器、重置状态

			// Tell the subclass to refresh the internal bean factory.
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory(); // 拿到beanFactory，里面读取了bean definition

			// Prepare the bean factory for use in this context.
			prepareBeanFactory(beanFactory); // 准备（准备什么？）

			try {
				// Allows post-processing of the bean factory in context subclasses.
				postProcessBeanFactory(beanFactory); // 后处理（应该是BeanFactory部分）

				StartupStep beanPostProcess = this.applicationStartup.start("spring.context.beans.post-process"); // 应该只是标记
				// Invoke factory processors registered as beans in the context.
				invokeBeanFactoryPostProcessors(beanFactory); // ？

				// Register bean processors that intercept bean creation.
				registerBeanPostProcessors(beanFactory); // 注册用来拦截对象创建的啥
				beanPostProcess.end();

				// Initialize message source for this context.
				initMessageSource(); // 消息

				// Initialize event multicaster for this context.
				initApplicationEventMulticaster(); // 生成周期时间回调注册

				// Initialize other special beans in specific context subclasses.
				onRefresh(); // Spring 自身的 Bean？

				// Check for listener beans and register them.
				registerListeners(); // 注册监听器？

				// Instantiate all remaining (non-lazy-init) singletons.
				finishBeanFactoryInitialization(beanFactory); // 实例化 Bean

				// Last step: publish corresponding event.
				finishRefresh(); // 最后的完成事件
			}
